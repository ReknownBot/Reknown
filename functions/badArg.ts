...
module.exports = (message: Message, argNum: number, desc: string): void => {
  if (!(message.channel as TextChannel).permissionsFor(message.guild.me).has('EMBED_LINKS')) return void message.channel.send(`:x: Argument **#${argNum}** was invalid. Here's what was wrong with it.\n\n**${desc}**`);
