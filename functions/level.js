module.exports = async (Client, message) => {
    ...
    if (!scoreCount)
        return Client.sql.query('INSERT INTO scores (userID, guildID, points, level) VALUES ($1, $2, $3, $4)', [message.author.id, message.guild.id, 1, 0]);
    ...
    if (curLevel > scoreCount.level) {
        ...
        const { rows } = await sql.query('SELECT * FROM levelrole WHERE guildID = $1 AND level <= $2', [message.guild.id, curLevel]);
        ...
        if (rows.length !== 0) {
            ...
            rows.forEach(r => {
                ...
                if (!message.guild.roles.get(r.roleid))
                    ...
                    return sql.query('DELETE FROM levelrole WHERE guildID = $1 AND roleID = $2', [message.guild.id, r.roleid]);
