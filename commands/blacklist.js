                ...
                async function blacklist(sMessage) {
                    ...
                    let prom = new Promise(resolve => {
                        message.member.roles.forEach(async role => {
                            let row = (await sql.query(`SELECT * FROM permissions WHERE roleID = '${role.id}' AND pName = 'blacklist' AND pCategory = 'mod'`)).rows[0];
                    ...
                    if (row) {
                        ...
                        collector.on('collect', collected => {
                            ...
                            sql.query(`INSERT INTO blacklist (userId, guildId, reason, by) VALUES ('${selectedMember.id}', '${message.guild.id}', '${client.escape(reason)}', '${client.escape(`${message.author.tag} (${message.author.id})`)}')`);
