module.exports.run = async (client, message, args) => {
  ...
  const user = await client.functions.parseMention.run(args[0], message.guild, { type: 'user', client: client }).catch(() => false);
  if (!user) return client.functions.badArg.run(message, 1, `I did not find a user by that query (${client.escMD(args[0])}).`);
