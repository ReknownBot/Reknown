                ...
                async function levelrolecmd(sMessage) {
                    ...
                    let prom = new Promise(resolve => {
                        message.member.roles.forEach(async role => {
                            let row = await sql.get('SELECT * FROM permissions WHERE roleID = ? AND pName = ? AND pCategory = ?', [role.id, "role", "level"]);
                    ...
                    let row2 = await sql.get('SELECT * FROM toggleLevel WHERE guildId = ?', [message.guild.id]);
                    ...
                    if (choices[0] === choice) { // Add
                        ...
                        let row3 = await sql.get("SELECT * FROM levelrole WHERE guildID = ? AND roleID = ?", [message.guild.id, role.id]);
                        ...
                        sql.run("INSERT INTO levelrole (guildID, roleID, level) VALUES (?, ?, ?)", [message.guild.id, role.id, level]);
                        ...
                        sql.run("DELETE FROM levelrole WHERE guildID = ? AND roleID = ?", [message.guild.id, role.id]);
                        client.editMsg(sMessage, "Successfully removed that role from the list.", message);
                    } else if (choices[2] === choice) { // Clear
                        let row3 = await sql.all("SELECT * FROM levelrole WHERE guildID = ?", [message.guild.id]);
                        if (!row3[0]) return client.editMsg(sMessage, "No roles are in the list currently!", message);
                        row3.forEach(row4 => {
                            sql.run("DELETE FROM levelrole WHERE guildID = ? AND roleID = ?", [message.guild.id, row4.roleID]);
                        });
                        ...
                        row3.forEach(row4 => {
                            if (!message.guild.roles.get(row4.roleID))
                                return sql.run("DELETE FROM levelrole WHERE guildID = ? AND roleID = ?");
