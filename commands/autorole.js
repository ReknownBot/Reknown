                ...
                async function autorole(sMessage) {
                    ...
                    let prom = new Promise((resolve, reject) => {
                        message.member.roles.forEach(async role => {
                            let row = await sql.get('SELECT * FROM permissions WHERE roleID = ? AND pName = ? AND pCategory = ?', [role.id, "autorole", "misc"]);
                    ...
                    if (args[1].toLowerCase() === "add") {
                        ...
                        let ro = await sql.get(`SELECT * FROM autorole WHERE roleId = ${selectedRole.id} AND guildId = ${message.guild.id}`);
                        ...
                        sql.run(`INSERT INTO autorole (guildId, roleId) VALUES (?, ?)`, [message.guild.id, selectedRole.id]);
                        ...
                        // If the role is not found by ID or there was no role mention
                        if (!selectedRole) return client.editMsg(sMessage, 'You have to mention or supply a valid ID of a role for me to set the auto role for this server!', message);
                        let r = await sql.get(`SELECT * FROM autorole WHERE roleId = ${selectedRole.id}`);
                        if (!r) return client.editMsg(sMessage, "The auto role does not exist!", message);
                        sql.run(`DELETE FROM autorole WHERE roleId = ${selectedRole.id}`);
                        ...
                        let rows = await sql.all(`SELECT * FROM autorole WHERE guildId = "${message.guild.id}"`);
                        if (Object.values(rows).length === 0) return client.editMsg(sMessage, `There are no auto roles set yet! Use ${PREFIX}autorole add <Mention or RoleID> to add one.`, message);
                        rows.forEach(() => {
                            sql.run(`DELETE FROM autorole WHERE guildId = ${message.guild.id}`);
                        });
                        client.editMsg(sMessage, "Successfully cleared the autorole.", message);
                    } else if (args[1].toLowerCase() === "list") {
                        let rows = await sql.all(`SELECT * FROM autorole WHERE guildId = ${message.guild.id}`);
                        if (Object.values(rows).length === 0) return client.editMsg(sMessage, "This guild does not have any auto roles!", message);
                        let x = [];
                        rows.forEach(r => {
