...
module.exports.run = async (client, message, args) => {
  ...
  if (!q) return client.functions.noArg.run(message, 1, 'a query to search for.');
  if (!sources.includes(branch.toLowerCase())) return client.functions.badArg.run(message, 2, `The source provided was invalid. It can be ${sources.join(', ')}.`);
  ...
  if (!embed) return client.functions.badArg.run(message, 1, 'I did not find anything with that query.');
