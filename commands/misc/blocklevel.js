...
module.exports = async (Client, message, args) => {
  if (!await Client.checkPerms('block', 'level')) return Client.functions.get('noCustomPerm')(message, 'level.block');
  ...
  const row = (await Client.sql.query('SELECT * FROM levelblock WHERE channelid = $1', [message.guild.id])).rows[0];
  let toggle;
  if (!args[2]) {
    if (!row) toggle = true;
  ...
  return message.channel.send(`Successfully ${toggle ? 'allowed' : 'blocked'} levelling up in ${channel}.`);
};
