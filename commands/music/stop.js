module.exports = async (Client, message, args) => {
    ...
    if (message.member.voice.channel !== message.guild.me.voice.channel)
        return message.reply('You have to be in the same voice channel as me to use this command!');

    // If a search is in progress
    if (guild.searching)
        return message.reply('Someone is searching a video right now, so you cannot use that command.');
    ...
    return message.reply('Successfully stopped the song.');
}
